...
#define SOUTHBOUND_IFC "enp10s0"  //送信NIC
#define NORTHBOUND_IFC "enp4s0u2" //受信NIC
...
PARAM Name = { NORTHBOUND_IFC, SOUTHBOUND_IFC };  //NIC NAME
DEVICE Device[4];
...
int Bridge() {
  int check, nready, i, size, optionLen, len;
  u_char* ptr;
	...
	if (targets[i].revents & (POLLIN | POLLERR)) {
	  if ((size = read(Device[i].soc, buf, sizeof(buf))) <= 0) {
		...
		if (iphdr->protocol == IPPROTO_UDP) {
	      ...
	      if ((size = write(Device[1].soc, buf, size)) <= 0) { //System write to outer raspi
		perror("write_Dev1");
	      }
	    }
	    else if (i == 1) {
