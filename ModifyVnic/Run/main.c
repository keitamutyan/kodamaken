...
//init.cは必要(MACアドレスを取得するため)

int main(int argc, char* argv[], char* envp[]) {

  getDevMACaddr(Name.Device1, DEVICE1_MAC); //init.cのgetDevMACaddr関数を使用して物理NICとvNICのMACアドレスを取得
  getDevMACaddr(Name.Device2, DEVICE2_MAC);
  
  if ((Device[0].soc = InitRawSocket(Name.Device1, 1, 0)) == -1) {  //今ここ
